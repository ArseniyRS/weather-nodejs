...
function printError(msg: string, lang = ELang.EN) {
  return console.log(chalk.bgRed(langDict[lang].error) + ' ' + msg)
}
function printSuccess(msg: string, lang = ELang.EN) {
  return console.log(chalk.bgGreen(langDict[lang].success) + ' ' + msg)
}
function printHelp(variant?: ECmds, lang = ELang.EN) {
...
function printWeather(data: IResponseWeather, lang = ELang.EN) {
  console.log(
    dedent(
      chalk.bgGreen(`${langDict[lang].now}:\n`) +
        `${langDict[lang].city}: ${data.name}
        ${langDict[lang].temperature}: ${data.main.temp}
        ${langDict[lang].weather}: ${getWeatherIcon(data.weather[0].icon)} ${
